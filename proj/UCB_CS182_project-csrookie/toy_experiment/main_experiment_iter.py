...
CONFIG = {
    ...
    "num_epochs_a": 2,  # Epochs to master Task A
    "num_epochs_b": 2,  # Epochs to learn Task B (and forget A)
    "num_epochs_joint": 2, # Epochs for the control group
    "learning_rate": 2e-5,
    "max_seq_length": 128, # Keep sequences short to save VRAM
    "eval_every_n_steps": 200, # Evaluate every 2000 iterations in Phase 2
...
def run_joint_training(loaders):
    ...
    for epoch in range(CONFIG["num_epochs_joint"]):
        ...
        metrics['step'] = (epoch + 1) * len(loaders["joint_train"])
...
def run_sequential_training(loaders):
    ...
    Phase 2: Train *only* on Task B, with fine-grained evaluation.
    """
    ...
    for epoch in range(CONFIG["num_epochs_a"]):
        ...
        train_one_epoch(model, loaders["a_train"], optimizer_a, scheduler_a, device)
        metrics = evaluate(model, loaders["a_val"], loaders["b_val"], device)
        print(f"  > Acc A (World/Sports): {metrics['acc_a']:.4f} (Loss: {metrics['loss_a']:.4f})")
        print(f"  > Acc B (Biz/Sci): {metrics['acc_b']:.4f} (Loss: {metrics['loss_b']:.4f})")
        metrics['step'] = (epoch + 1) * len(loaders["a_train"])
        history.append(metrics)

    print(f"\n  --- Finished Phase 1. Model is now an 'expert' on Task A. ---\n")

    # --- Phase 2: Train on Task B with fine-grained evaluation ---
    print("  --- Phase 2: Training on Task B (Business/Sci) ---")
    print(f"  *** Evaluating every {CONFIG['eval_every_n_steps']} steps. WATCH FORGETTING IN SLOW-MOTION ***")
    ...
    model.train()
    progress_bar = tqdm(total=num_training_steps_b, desc="Phase 2 Training")
    
    global_step = CONFIG["num_epochs_a"] * len(loaders["a_train"])
    
    for epoch in range(CONFIG["num_epochs_b"]):
        for batch in loaders["b_train"]:
            ...
            if global_step % CONFIG["eval_every_n_steps"] == 0:
                ...
                progress_bar.set_postfix({
                    "Acc A": f"{metrics['acc_a']:.3f}", 
                    "Acc B": f"{metrics['acc_b']:.3f}"
                })
...
def plot_results(joint_history, seq_history):
    ...
    def get_metrics(history):
        ...
        return steps, acc_a, loss_a, acc_b, loss_b

    j_steps, j_acc_a, j_loss_a, j_acc_b, j_loss_b = get_metrics(joint_history)
    s_steps, s_acc_a, s_loss_a, s_acc_b, s_loss_b = get_metrics(seq_history)

    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle("Catastrophic Forgetting: Joint vs. Sequential Training on AG News", fontsize=16)

    task_a_end_step = CONFIG["num_epochs_a"] * len(all_loaders["a_train"])

    # --- Plot 1: Task A Accuracy (The Forgetting Curve) ---
    ax1 = axes[0, 0]
    ax1.plot(j_steps, j_acc_a, 'o-', label="Joint Training (Control)")
    ax1.plot(s_steps, s_acc_a, 'o-', label="Sequential Training (CF)")
    ax1.axvline(x=task_a_end_step, color='r', linestyle='--', label="Task B Starts")
    ax1.set_title("Task A (World/Sports) Accuracy")
    ax1.set_ylabel("Accuracy")
    ax1.set_xlabel("Training Steps (Iterations)")
    ax1.legend()
    ax1.grid(True)

    # --- Plot 2: Task B Accuracy (The Learning Curve) ---
    ax2 = axes[0, 1]
    ax2.plot(j_steps, j_acc_b, 'o-', label="Joint Training (Control)")
    ax2.plot(s_steps, s_acc_b, 'o-', label="Sequential Training (CF)")
    ax2.axvline(x=task_a_end_step, color='r', linestyle='--', label="Task B Starts")
    ax2.set_title("Task B (Business/Sci) Accuracy")
    ax2.set_ylabel("Accuracy")
    ax2.set_xlabel("Training Steps (Iterations)")
    ax2.legend()
    ax2.grid(True)

    # --- Plot 3: Task A Loss (The Forgetting Curve) ---
    ax3 = axes[1, 0]
    ax3.plot(j_steps, j_loss_a, 'o-', label="Joint Training (Control)")
    ax3.plot(s_steps, s_loss_a, 'o-', label="Sequential Training (CF)")
    ax3.axvline(x=task_a_end_step, color='r', linestyle='--', label="Task B Starts")
    ax3.set_title("Task A (World/Sports) Loss")
    ax3.set_ylabel("Loss")
    ax3.set_xlabel("Training Steps (Iterations)")
    ax3.legend()
    ax3.grid(True)
    ax3.set_yscale('log')
    ...
    ax4 = axes[1, 1]
    ax4.plot(j_steps, j_loss_b, 'o-', label="Joint Training (Control)")
    ax4.plot(s_steps, s_loss_b, 'o-', label="Sequential Training (CF)")
    ax4.axvline(x=task_a_end_step, color='r', linestyle='--', label="Task B Starts")
    ax4.set_title("Task B (Business/Sci) Loss")
    ax4.set_ylabel("Loss")
    ax4.set_xlabel("Training Steps (Iterations)")
    ax4.legend()
    ax4.grid(True)
    
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig("catastrophic_forgetting_results_smooth.png")
    print("\nResults saved to 'catastrophic_forgetting_results_smooth.png'")
    plt.show()
